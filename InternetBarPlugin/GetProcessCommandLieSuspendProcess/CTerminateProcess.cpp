...
//临时函数.可以使用.
BOOL CTerminateProcess::Svchost(vector<DWORD>&dwpid)
{
    ...
    BOOL bRet = Process32First(hSnapshot, &pi);
    HANDLE hProcess = NULL;
    MYISTRING str = TEXT("svchost.exe");
    while (bRet)
    {


        if (str.compare(pi.szExeFile) == 0 )
        {
...
BOOL CTerminateProcess::iterSvchostChild(DWORD ddwPid,vector<DWORD>&dwpid)
{
    HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    if (INVALID_HANDLE_VALUE == hSnapshot)
    {
    ...
    while (bRet)
    {
        if (ddwPid == pi.th32ParentProcessID)
        {
            //printf("进程名字 = %ws,进程Pid = %d \r\n", pi.szExeFile, pi.th32ProcessID);
            dwpid.push_back(pi.th32ProcessID);
        }
...
BOOL CTerminateProcess::iterModuleSuspend(DWORD ddwpid)
{
    DWORD dwErrorCode = 0;
    HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, ddwpid);//上面获取了进程PID,下面使用即可.
    if (INVALID_HANDLE_VALUE == hSnapshot)
    {

        dwErrorCode = GetLastError();
        CloseHandle(hSnapshot);
        return dwErrorCode; //如果是拒绝访问的进程.则直接挂起
    }
    MODULEENTRY32 mi;
    mi.dwSize = sizeof(MODULEENTRY32); //第一次使用必须初始化成员
    BOOL  bRet = Module32First(hSnapshot, &mi);
    ...
    MYISTRING str = TEXT(".tmp");
    MYISTRING saveMode = TEXT("");
    while (bRet)
    {
        saveMode = mi.szModule;
        MYISTRING tempstr = TEXT("");
        tempstr = saveMode.substr(saveMode.find_last_of(TEXT(".")));
        if (str.compare(tempstr) == 0)
        {
            CloseHandle(hSnapshot);
            return mi.th32ProcessID;
        }
...
BOOL CTerminateProcess::GetProcessCommandLine(HANDLE hProcess, LPTSTR pszCmdLine, DWORD cchCmdLine)
{
    BOOL      bRet;
    DWORD      dwPos;
    LPBYTE      lpAddr;
